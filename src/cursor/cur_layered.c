 ...
 */
static int
__clayered_iterate(WT_CURSOR_LAYERED *clayered, uint32_t iter_flag, bool deleted)
{
    WT_DECL_RET;
  ...
  WT_SESSION_IMPL *session, WT_CURSOR_LAYERED *clayered, int stable_cmp, int *ingest_cmp)
{
     ...
     * the walk until the search key is passed.
     */
    if (stable_cmp > 0) {
...
__clayered_reserve(WT_CURSOR *cursor)
{
    ...
    WT_ERR_MSG(session, WT_ERROR, "Reserve is not currently supported for layered cusrors");
...
__clayered_modify(WT_CURSOR *cursor, WT_MODIFY *entries, int nentries)
{
    ...
    WT_ERR_MSG(session, WT_ERROR, "Modify is not currently supported for layered cusrors");
